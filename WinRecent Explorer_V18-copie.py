...
- NOUVEAU : Clic droit sur une ligne -> copie le champ "Nom" dans le presse-papiers
"""
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Double-clic (inchangé) → ouvrir le .lnk dans Explorer
        self.tree.bind("<Double-1>", self._on_double_click_row)
        # Clic droit → copier le champ "Nom" de la ligne
    ...
    def _on_right_click_copy_name(self, event):
        """Copie le champ 'Nom' de la ligne cliquée dans le presse-papiers (clic droit)."""
        row_id = self.tree.identify_row(event.y)
        if not row_id:
            return
        # sélectionne visuellement la ligne
        self.tree.selection_set(row_id)
        self.tree.focus(row_id)
        ...
        try:
            ...
            self.update()  # pour fiabiliser la mise à jour du clipboard
