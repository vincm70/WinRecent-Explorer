# -*- coding: utf-8 -*-
r"""
Historique des éléments consultés (Windows) – GUI Tkinter
- Parcourt %AppData%\Microsoft\Windows\Recent
- Résout les .lnk via COM (ctypes) -> pas de pywin32
- Stocke en SQLite (historique persistant)
- Double-clic/bouton "Ouvrir (dans 'Recent')" -> ouvre le .lnk dans l'Explorateur
- Bouton "Activer la sauvegarde Auto (tâche planifiée)" -> crée une tâche Windows hebdomadaire
  qui lance ce script avec l'option --weekly-scan (scan + mise à jour DB, sans GUI)
"""
...
TASK_NAME = "RecentHistory_AutoScanWeekly"  # nom de la tâche planifiée
...
if not hasattr(wt, "LPCOLESTR"):
    wt.LPCOLESTR = wt.LPWSTR  # Unicode paths only → OK
...
def resolve_lnk(lnk_path: Path) -> str:
    ...
    try:
        ...
        SLGP_RAWPATH = 0x0000
        if get_path(shell_link, buf, 260, None, SLGP_RAWPATH) != 0:
            return ""
        return buf.value or ""
    except Exception:
        return ""
...
def create_weekly_task():
    """
    Crée/remplace une tâche planifiée Windows qui exécute le script en mode --weekly-scan
    tous les lundis à 09:00 (heure locale de la machine).
    """
    python_exe = sys.executable
    script_path = Path(__file__).resolve()
    # Commande que la tâche exécutera (Shell)
    tr_cmd = f'"{python_exe}" "{script_path}" --weekly-scan'

    try:
        # /F pour forcer le remplacement si la tâche existe
        # /SC WEEKLY /D MON /ST 09:00 => Hebdo, lundi 09:00
        subprocess.run([
            "schtasks", "/Create",
            "/TN", TASK_NAME,
            "/F",
        ...
        return True, f"Tâche planifiée créée/actualisée : {TASK_NAME}\nExécution: chaque lundi 09:00."
    except subprocess.CalledProcessError as e:
        return False, f"Échec de la création de la tâche.\nCode: {e.returncode}"
    except FileNotFoundError:
        return False, "Commande 'schtasks' introuvable (Windows requis)."

def run_weekly_scan_once():
    """
    Mode headless : scan + maj DB puis sortie.
    Utilisé par la tâche planifiée (--weekly-scan).
    """
    con = ensure_db()
    n = scan_recent(con)
    # en option on peut écrire un mini log
    try:
        DB_DIR.mkdir(parents=True, exist_ok=True)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Historique des éléments consultés")
        ...
        self.tree.heading("target_path", text="Chemin")
    ...
    def enable_autoscan(self):
        """Crée/actualise la tâche planifiée hebdomadaire (scan DB)."""
...
if __name__ == "__main__":
    if "--weekly-scan" in sys.argv:
        # Mode tâche planifiée : scan + sortie (pas d'UI)
