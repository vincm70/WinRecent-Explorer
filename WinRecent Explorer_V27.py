...
- Double-clic : sélectionne la ligne + ouvre le .lnk correspondant dans l’Explorateur (dossier Recent)
- Bouton "Ouvrir (dans 'Recent')" : ouvre le .lnk basé sur la ligne sélectionnée
- Nouveau : bouton "Ouvrir la cible" -> ouvre directement la cible du .lnk (os.startfile)
...
# ===================== COM / ShellLink (réservé si on veut plus tard) =====================
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Interactions
        self.tree.bind("<Double-1>", self._on_double_click_row)      # sélection + ouverture (dans Recent)
    ...
    def _on_double_click_row(self, event):
        """Double-clic : sélectionne la ligne + ouvre selon cette sélection (dans Recent)."""
    ...
    def open_file(self):
        """Bouton + double-clic : ouvre le .lnk dans l'explorateur (dossier Recent)."""
    ...
    def open_target(self):
        """NOUVEAU : ouvre directement la cible du raccourci (optionnel)."""
        name = self._get_selected_name()
        if not name:
            messagebox.showwarning(APP_NAME, "Aucune sélection.")
            return
